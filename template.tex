...
\abstract{Reflectance Confocal Microscopy is an appropriate tool for diagnosis of the diagnosis of Lentigo Maligna and Lentigo Maligna Melanoma. In comparison with Dermoscopy, this device can provide rich information as a mosaic and/or stack of images. In this particular context, the number of images by patients varies between 2 and 833 images and the finality is to perform detection of these between Benign and Malignant classes. First, this paper evaluates classification at image-level with help of handcrafted methods issue from literature and Transfer Learning methods. Second, this work propose patient-level supervised methods based on images decisions and compare them to multi-instance learning methods. At image-level classification, this study achieve a F1-Score of 0.83 on malignant pathologies by use of “Inception-ResNet” architecture and Linear SVM. On patient-level side, this study reach F1-Score of 0.84 for Dynamic threshold over Decisions and 0.88 for multi-instance learning based on MI-SVM.}\par
...
In order to reduce the imbalancement of the data, 28 benign patients are also provided by dermatologist, that varies between 4 and 103 images by patient (608 images in total). These new patients are only considered for a training purpose and be not take into account into \Cref{sec:results}.\par
...
\begin{table}[h]
    ...
    \label{tab:patient_results}
\end{table}\par
As Recall of positive class refer to sensitivity and Recall of negative class refer to specificity we get all information to compare our results with dermatologists.  \Cref{patient_results_details} \Cref{roc_results}
